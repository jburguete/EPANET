...
*/
{
  ...
  if (qual->Qualflag == TRACE)
    qual->NodeQual[qual->TraceNode] = 100.0;

  // Compute Schmidt number
  if (qual->Diffus > 0.0)
    qual->Sc = hyd->Viscos / qual->Diffus;
  else
    qual->Sc = 0.0;
      ...
      if (qual->Qualflag != NONE && time->Qtime < time->Dur)
        {
          ...
          if (qual->Reactflag && qual->Qualflag != AGE)
            ratecoeffs (pr);
...
int
nextqual (Project * pr, long *tstep)
/*
**--------------------------------------------------------------
**   Input:   none
...
int
stepqual (Project * pr, long *tleft)
/*
**--------------------------------------------------------------
**   Input:   none
...
double
findsourcequal (Project * pr, int n, double volout, long tstep)
/*
**---------------------------------------------------------------------
**   Input:   n = node index
...
double
sourcequal (Project * pr, Psource source)
/*
**--------------------------------------------------------------
  ...
  if (qual->Qualflag == NONE)
  ...
  else
    {
      ...
      if (massin == 0.0)
        ...
        qual->MassBalance.ratio = massout / massin;
    }
}


double
findstoredmass (Project * pr)
/*
**--------------------------------------------------------------
**   Input:   none
