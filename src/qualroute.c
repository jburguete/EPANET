...
void
transport (Project * pr, long tstep)
/*
**--------------------------------------------------------------
...
void
evalnodeinflow (Project * pr, int k, long tstep, double *volin, double *massin)
/*
**--------------------------------------------------------------
**   Input:   k = link index
...
double
findnodequal (Project * pr, int n, double volin,
  ...
  if (net->Node[n].Type == JUNCTION)
    {
      ...
      if (volin > 0.0)
        qual->NodeQual[n] = massin / volin;

      // ... if no inflow adjust quality for reaction in connecting pipes
      else if (qual->Reactflag)
        qual->NodeQual[n] = noflowqual (pr, n);
    }
      ...
      if (n == qual->TraceNode)
        {
          ...
          if (net->Node[n].Type == RESERVOIR)
            qual->SourceQual = 100.0;
          else
            qual->SourceQual = MAX (100.0 - qual->NodeQual[n], 0.0);
  ...
  for (alink = net->Adjlist[n]; alink != NULL; alink = alink->next)
    {
      ...
      if (net->Link[k].N2 == n && dir >= 0)
        inflow = TRUE;
      else if (net->Link[k].N1 == n && dir < 0)
        inflow = TRUE;
      else
        inflow = FALSE;
...
void
evalnodeoutflow (Project * pr, int k, double c, long tstep)
/*
**--------------------------------------------------------------
**   Input:   k = link index
...
void
updatemassbalance (Project * pr, int n, double massin,
...
*/
{
  ...
  double masslost = 0.0, massadded = 0.0;
  ...
  switch (net->Node[n].Type)
    ...
    case RESERVOIR:
      ...
      if (qual->SourceQual > 0.0)
        ...
        massadded = qual->NodeQual[n] * volout;
      break;
...
int
selectnonstacknode (Project * pr, int numsorted, int *indegree)
/*
**--------------------------------------------------------------
...
void
addseg (Project * pr, int k, double v, double c)
/*
**-------------------------------------------------------------
