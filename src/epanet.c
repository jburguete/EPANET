...
*/
{
  struct Project *project =
    (struct Project *) calloc (1, sizeof (struct Project));
  if (project == NULL)
    return -1;
...
int DLLEXPORT
EN_runproject (EN_Project p, const char *inpFile,
...
int DLLEXPORT
EN_init (EN_Project p, const char *rptFile, const char *outFile,
...
int DLLEXPORT
EN_open (EN_Project p, const char *inpFile, const char *rptFile,
         const char *outFile)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_gettitle (EN_Project p, char *line1, char *line2, char *line3)
/*----------------------------------------------------------------
**  Input:   None
...
int DLLEXPORT
EN_settitle (EN_Project p, char *line1, char *line2, char *line3)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_getcomment (EN_Project p, int object, int index, char *comment)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_setcomment (EN_Project p, int object, int index, char *comment)
/*----------------------------------------------------------------
**  Input:   object = a type of object (see EN_ObjectType)
**           index = the object's index
...
int DLLEXPORT
EN_getcount (EN_Project p, int object, int *count)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_saveinpfile (EN_Project p, const char *filename)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_initH (EN_Project p, int initFlag)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_runH (EN_Project p, long *currentTime)
/*----------------------------------------------------------------
**  Input:   none
...
int DLLEXPORT
EN_nextH (EN_Project p, long *tStep)
/*----------------------------------------------------------------
**  Input:   none
...
int DLLEXPORT
EN_savehydfile (EN_Project p, const char *filename)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_usehydfile (EN_Project p, const char *filename)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_initQ (EN_Project p, int saveFlag)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_runQ (EN_Project p, long *currentTime)
/*----------------------------------------------------------------
**  Input:   none
...
int DLLEXPORT
EN_nextQ (EN_Project p, long *tStep)
/*----------------------------------------------------------------
**  Input:   none
...
int DLLEXPORT
EN_stepQ (EN_Project p, long *timeLeft)
/*----------------------------------------------------------------
**  Input:   none
...
int DLLEXPORT
EN_writeline (EN_Project p, char *line)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_copyreport (EN_Project p, char *filename)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_setreport (EN_Project p, char *format)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_setstatusreport (EN_Project p, int level)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_geterror (int errcode, char *errmsg, int maxLen)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_getstatistic (EN_Project p, int type, double *value)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_getresultindex (EN_Project p, int type, int index, int *value)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_getoption (EN_Project p, int option, double *value)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_setoption (EN_Project p, int option, double value)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_getflowunits (EN_Project p, int *units)
/*----------------------------------------------------------------
**  Input:   none
...
int DLLEXPORT
EN_setflowunits (EN_Project p, int units)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_gettimeparam (EN_Project p, int param, long *value)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_settimeparam (EN_Project p, int param, long value)
/*----------------------------------------------------------------
**  Input:   param = time parameter code (see EN_TimeParameter)
...
int DLLEXPORT
EN_getqualinfo (EN_Project p, int *qualType, char *chemName,
...
int DLLEXPORT
EN_getqualtype (EN_Project p, int *qualType, int *traceNode)
/*----------------------------------------------------------------
**  Input:   none
**  Output:  qualType = type of quality analysis to run (see EN_QualityType)
...
int DLLEXPORT
EN_setqualtype (EN_Project p, int qualType, char *chemName,
...
int DLLEXPORT
EN_addnode (EN_Project p, char *id, int nodeType, int *index)
/*----------------------------------------------------------------
  ...
  *index = 0;
  ...
  // Actions taken when a new Tank/Reservoir is added
  else
    {
      ...
      // resize tanks array
      net->Tank =
        (Stank *) realloc (net->Tank, (net->Ntanks + 1) * sizeof (Stank));
...
int DLLEXPORT
EN_deletenode (EN_Project p, int index, int actionCode)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_getnodeindex (EN_Project p, char *id, int *index)
/*----------------------------------------------------------------
**  Input:   id = node ID name
...
int DLLEXPORT
EN_getnodeid (EN_Project p, int index, char *id)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_setnodeid (EN_Project p, int index, char *newid)
/*----------------------------------------------------------------
**  Input:   index = node index
...
*/
{
  ...
  if (g_hash_table_lookup (net->NodeHashTable, newid))
    return 215;
  ...
  g_hash_table_insert (net->NodeHashTable, net->Node[index].ID,
                       GINT_TO_POINTER (index));
  return 0;
}

int DLLEXPORT
EN_getnodetype (EN_Project p, int index, int *nodeType)
/*----------------------------------------------------------------
**  Input:   index = node index
...
int DLLEXPORT
EN_getnodevalue (EN_Project p, int index, int property, double *value)
/*----------------------------------------------------------------
**  Input:   index = node index
  ...
  switch (property)
    ...
    case EN_EMITTER:
      ...
      if (Node[index].Ke > 0.0)
        {
          v =
            Ucf[FLOW] / pow ((Ucf[PRESSURE] * Node[index].Ke),
                             (1.0 / hyd->Qexp));
        }
      break;
...
int DLLEXPORT
EN_setnodevalue (EN_Project p, int index, int property, double value)
/*----------------------------------------------------------------
**  Input:   index = node index
**           property  = node property code (see EN_NodeProperty)
...
*/
{
  ...
  switch (property)
      ...
      if (i > 0)                // tank has a volume curve
        {
          ...
          if (hTmp > Tank[j].H0 || hTmp > Tank[j].Hmax)
    ...
    case EN_VOLCURVE:
      ...
      if (i <= 0 || i > net->Ncurves)
        return 205;             // invalid curve index
      if (index <= nJuncs)
        return 0;               // node not a tank
      j = index - nJuncs;       // tank index
      if (Tank[j].A == 0.0)
        return 0;               // tank is a reservoir
      curve = &net->Curve[i];   // curve object
...
int DLLEXPORT
EN_setjuncdata (EN_Project p, int index, double elev,
...
int DLLEXPORT
EN_settankdata (EN_Project p, int index, double elev,
...
int DLLEXPORT
EN_getcoord (EN_Project p, int index, double *x, double *y)
/*----------------------------------------------------------------
**  Input:   index = node index
...
*/
{
  ...
  node = &net->Node[index];
  if (node->X == MISSING || node->Y == MISSING)
    return 254;
...
int DLLEXPORT
EN_setcoord (EN_Project p, int index, double x, double y)
/*----------------------------------------------------------------
**  Input:   index = node index
**           x = node x-coordinate
**           y = node y-coordinate
...
int DLLEXPORT
EN_getdemandmodel (EN_Project p, int *model, double *pmin,
...
int DLLEXPORT
EN_setdemandmodel (EN_Project p, int model, double pmin,
                   double preq, double pexp)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_adddemand (EN_Project p, int nodeIndex, double baseDemand,
...
int DLLEXPORT
EN_deletedemand (EN_Project p, int nodeIndex, int demandIndex)
/*----------------------------------------------------------------
**  Input:   nodeIndex = node index
...
int DLLEXPORT
EN_getdemandindex (EN_Project p, int nodeIndex, char *demandName,
                   int *demandIndex)
/*----------------------------------------------------------------
**  Input:   nodeIndex = node index
...
int DLLEXPORT
EN_getnumdemands (EN_Project p, int nodeIndex, int *numDemands)
/*----------------------------------------------------------------
**  Input:   nodeIndex = node index
...
int DLLEXPORT
EN_getbasedemand (EN_Project p, int nodeIndex, int demandIndex,
                  double *baseDemand)
/*----------------------------------------------------------------
**  Input:   nodeIndex = node index
...
int DLLEXPORT
EN_setbasedemand (EN_Project p, int nodeIndex, int demandIndex,
                  double baseDemand)
/*----------------------------------------------------------------
**  Input:   nodeIndex = node index
**           demandIndex = demand category index
...
int DLLEXPORT
EN_getdemandname (EN_Project p, int nodeIndex, int demandIndex,
                  char *demandName)
/*----------------------------------------------------------------
**  Input:   nodeIndex = node index
**           demandIndex = demand category index
...
int DLLEXPORT
EN_setdemandname (EN_Project p, int nodeIndex, int demandIndex,
                  char *demandName)
/*----------------------------------------------------------------
**  Input:   nodeIndex = node index
**           demandIndex = demand category index
...
int DLLEXPORT
EN_getdemandpattern (EN_Project p, int nodeIndex, int demandIndex,
                     int *patIndex)
/*----------------------------------------------------------------
**  Input:   nodeIndex = node index
**           demandIndex = demand category index
...
int DLLEXPORT
EN_setdemandpattern (EN_Project p, int nodeIndex, int demandIndex, int patIndex)
/*----------------------------------------------------------------
**  Input:   nodeIndex = node index
**           demandIndex = demand category index
...
int DLLEXPORT
EN_addlink (EN_Project p, char *id, int linkType,
...
*/
{
  ...
  *index = 0;
  ...
  if (linkType < CVPIPE || linkType > GPV)
    return 251;
...
int DLLEXPORT
EN_deletelink (EN_Project p, int index, int actionCode)
/*----------------------------------------------------------------
  ...
  for (i = index; i <= net->Nlinks - 1; i++)
    {
      ...
      g_hash_table_replace (net->LinkHashTable, net->Link[i].ID,
                            GINT_TO_POINTER (i));
    }
...
int DLLEXPORT
EN_getlinkindex (EN_Project p, char *id, int *index)
/*----------------------------------------------------------------
**  Input:   id = link ID name
...
int DLLEXPORT
EN_getlinkid (EN_Project p, int index, char *id)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_setlinkid (EN_Project p, int index, char *newid)
/*----------------------------------------------------------------
**  Input:   index = link index
**           id = link ID name
**  Output:  none
...
*/
{
  ...
  if (g_hash_table_lookup (net->LinkHashTable, newid))
    return 215;
  ...
  g_hash_table_insert (net->LinkHashTable, net->Link[index].ID,
                       GINT_TO_POINTER (index));
  return 0;
}

int DLLEXPORT
EN_getlinktype (EN_Project p, int index, int *linkType)
/*----------------------------------------------------------------
**  Input:   index = link index
...
int DLLEXPORT
EN_setlinktype (EN_Project p, int *index, int linkType, int actionCode)
/*----------------------------------------------------------------
**  Input:   index = link index
...
int DLLEXPORT
EN_getlinknodes (EN_Project p, int index, int *node1, int *node2)
/*----------------------------------------------------------------
**  Input:   index = link index
...
int DLLEXPORT
EN_setlinknodes (EN_Project p, int index, int node1, int node2)
/*----------------------------------------------------------------
**  Input:   index = link index
...
int DLLEXPORT
EN_getlinkvalue (EN_Project p, int index, int property, double *value)
/*----------------------------------------------------------------
**  Input:   index = link index
      ...
      if (Link[index].Type == PUMP)
        {
          ...
          if (hyd->LinkStatus[index] >= OPEN)
            {
              if (hyd->LinkFlow[index] >
                  hyd->LinkSetting[index] * Pump[pmp].Qmax)
                {
                  v = XFLOW;
                }
...
int DLLEXPORT
EN_setlinkvalue (EN_Project p, int index, int property, double value)
/*----------------------------------------------------------------
**  Input:   index = link index
**           property  = link property code (see EN_LinkProperty)
      ...
      if (Link[index].Type == PIPE || Link[index].Type == CVPIPE)
      ...
      else
        {
          ...
          if (property == EN_INITSETTING)
            {
              setlinksetting (p, index, value, &Link[index].Status,
                              &Link[index].Kc);
            }
          else
            {
...
int DLLEXPORT
EN_setpipedata (EN_Project p, int index, double length,
...
*/
{
  ...
  Link[index].Km =
    0.02517 * mloss / SQR (Link[index].Diam) / SQR (Link[index].Diam);
  resistcoeff (p, index);
  return 0;
}

int DLLEXPORT
EN_getvertexcount (EN_Project p, int index, int *count)
/*----------------------------------------------------------------
**  Input:   index = link index
...
int DLLEXPORT
EN_getvertex (EN_Project p, int index, int vertex, double *x, double *y)
/*----------------------------------------------------------------
**  Input:   index = link index
...
int DLLEXPORT
EN_setvertices (EN_Project p, int index, double *x, double *y, int count)
/*----------------------------------------------------------------
**  Input:   index = link index
...
int DLLEXPORT
EN_getpumptype (EN_Project p, int linkIndex, int *pumpType)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_getheadcurveindex (EN_Project p, int linkIndex, int *curveIndex)
/*----------------------------------------------------------------
**  Input:   linkIndex = index of a pump link
...
int DLLEXPORT
EN_setheadcurveindex (EN_Project p, int linkIndex, int curveIndex)
/*----------------------------------------------------------------
**  Input:   linkIndex = index of a pump link
...
int DLLEXPORT
EN_addpattern (EN_Project p, char *id)
/*----------------------------------------------------------------
...
*/
{
  ...
  n = net->Npats + 1;
  net->Pattern =
    (Spattern *) realloc (net->Pattern, (n + 1) * sizeof (Spattern));
...
int DLLEXPORT
EN_deletepattern (EN_Project p, int index)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_getpatternindex (EN_Project p, char *id, int *index)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_getpatternid (EN_Project p, int index, char *id)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_setpatternid (EN_Project p, int index, char *id)
/*----------------------------------------------------------------
**  Input:   index = time pattern index
...
int DLLEXPORT
EN_getpatternlen (EN_Project p, int index, int *len)
/*----------------------------------------------------------------
**  Input:   index = time pattern index
...
int DLLEXPORT
EN_getpatternvalue (EN_Project p, int index, int period, double *value)
/*----------------------------------------------------------------
**  Input:   index = time pattern index
...
int DLLEXPORT
EN_setpatternvalue (EN_Project p, int index, int period, double value)
/*----------------------------------------------------------------
**  Input:   index = time pattern index
**           period = time pattern period
...
int DLLEXPORT
EN_getaveragepatternvalue (EN_Project p, int index, double *value)
/*----------------------------------------------------------------
**  Input:   index = time pattern index
...
int DLLEXPORT
EN_setpattern (EN_Project p, int index, double *values, int len)
/*----------------------------------------------------------------
**  Input:   index = time pattern index
...
*/
{
  ...
  Pattern[index].F =
    (double *) realloc (Pattern[index].F, len * sizeof (double));
...
int DLLEXPORT
EN_addcurve (EN_Project p, char *id)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_deletecurve (EN_Project p, int index)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_getcurveindex (EN_Project p, char *id, int *index)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_getcurveid (EN_Project p, int index, char *id)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_setcurveid (EN_Project p, int index, char *id)
/*----------------------------------------------------------------
**  Input:   index = data curve index
...
int DLLEXPORT
EN_getcurvelen (EN_Project p, int index, int *len)
/*----------------------------------------------------------------
**  Input:   index = data curve index
...
int DLLEXPORT
EN_getcurvetype (EN_Project p, int index, int *type)
/*----------------------------------------------------------------
**  Input:   index = data curve index
...
int DLLEXPORT
EN_getcurvevalue (EN_Project p, int curveIndex, int pointIndex,
                  double *x, double *y)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_setcurvevalue (EN_Project p, int curveIndex, int pointIndex,
                  double x, double y)
/*----------------------------------------------------------------
**  Input:   curveIndex = data curve index
**           pointIndex = index of a data point on the curve
...
int DLLEXPORT
EN_getcurve (EN_Project p, int index, char *id, int *nPoints,
...
int DLLEXPORT
EN_setcurve (EN_Project p, int index, double *xValues,
...
int DLLEXPORT
EN_addcontrol (EN_Project p, int type, int linkIndex, double setting,
...
*/
{
  ...
  if (net->Link[linkIndex].Type == CVPIPE)
    return 207;
  ...
  if (type == EN_LOWLEVEL || type == EN_HILEVEL)
    {
      if (nodeIndex < 1 || nodeIndex > net->Nnodes)
        return 203;
    }
  else
    nodeIndex = 0;
  ...
  if (type == LOWLEVEL || type == HILEVEL)
    {
      if (nodeIndex > net->Njuncs)
        lvl = net->Node[nodeIndex].El + level / Ucf[ELEV];
      else
        lvl = net->Node[nodeIndex].El + level / Ucf[PRESSURE];
    }
  if (type == TIMER)
    t = (long) ROUND (lvl);
  if (type == TIMEOFDAY)
  ...
  n = net->Ncontrols + 1;
  net->Control =
    (Scontrol *) realloc (net->Control, (n + 1) * sizeof (Scontrol));
...
int DLLEXPORT
EN_deletecontrol (EN_Project p, int index)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_getcontrol (EN_Project p, int index, int *type, int *linkIndex,
...
int DLLEXPORT
EN_setcontrol (EN_Project p, int index, int type, int linkIndex,
               double setting, int nodeIndex, double level)
/*----------------------------------------------------------------
**  Input:   index  = index of the control
...
int DLLEXPORT
EN_addrule (EN_Project p, char *rule)
/*----------------------------------------------------------------
  ...
  while (line)
    {
      ...
      parser->Ntokens =
        gettokens (line2, parser->Tok, MAXTOKS, parser->Comment);
...
int DLLEXPORT
EN_deleterule (EN_Project p, int index)
/*----------------------------------------------------------------
...
int DLLEXPORT
EN_getrule (EN_Project p, int index, int *nPremises,
...
int DLLEXPORT
EN_getruleID (EN_Project p, int index, char *id)
/*----------------------------------------------------------------
**  Input:   index = rule index
...
int DLLEXPORT
EN_getpremise (EN_Project p, int ruleIndex, int premiseIndex,
...
int DLLEXPORT
EN_setpremise (EN_Project p, int ruleIndex, int premiseIndex,
               int logop, int object, int objIndex, int variable,
               int relop, int status, double value)
/*----------------------------------------------------------------
**  Input:   ruleIndex = rule index
...
int DLLEXPORT
EN_setpremiseindex (EN_Project p, int ruleIndex, int premiseIndex, int objIndex)
/*----------------------------------------------------------------
**  Input:   ruleIndex = rule index
**           premiseIndex = premise index
**           objIndex = object's index in Node or Link array
...
int DLLEXPORT
EN_setpremisestatus (EN_Project p, int ruleIndex, int premiseIndex, int status)
/*----------------------------------------------------------------
**  Input:   ruleIndex = rule index
**           premiseIndex = premise index
...
int DLLEXPORT
EN_setpremisevalue (EN_Project p, int ruleIndex, int premiseIndex, double value)
/*----------------------------------------------------------------
**  Input:   ruleIndex = rule index
**           premiseIndex = premise index
...
int DLLEXPORT
EN_getthenaction (EN_Project p, int ruleIndex, int actionIndex,
...
int DLLEXPORT
EN_setthenaction (EN_Project p, int ruleIndex, int actionIndex,
                  int linkIndex, int status, double setting)
/*----------------------------------------------------------------
**  Input:   ruleIndex = rule index
**           actionIndex = index of a rule's THEN actions
...
int DLLEXPORT
EN_getelseaction (EN_Project p, int ruleIndex, int actionIndex,
                  int *linkIndex, int *status, double *setting)
/*----------------------------------------------------------------
**  Input:   ruleIndex = rule index
...
int DLLEXPORT
EN_setelseaction (EN_Project p, int ruleIndex, int actionIndex,
                  int linkIndex, int status, double setting)
/*----------------------------------------------------------------
**  Input:   ruleIndex = rule index
**           actionIndex = index of a rule's ELSE actions
...
int DLLEXPORT
EN_setrulepriority (EN_Project p, int index, double priority)
/*-----------------------------------------------------------------------------
**  Input:   index  = rule index
