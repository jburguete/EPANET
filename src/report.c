/**
 * \fire report.c
 * \brief Source file with functions for writing formatted text to a report
 * file.
 * \author see AUTHORS.
...
/**
 * function to convert time in seconds to hours:minutes format.
 *
 * \return pointer to atime.
 */
char *
clocktime (char *atime,         ///< time in hrs:min.
...
/**
 * function to marks all junction nodes connected to tanks.
 */
static inline void
marknodes (Network * net,
           Hydraul * hyd,
           int m,               ///< number of source nodes.
           int *nodelist,       ///< list of nodes to be traced from.
           char *marked)        ///< TRUE if node connected to source.
{
  int i, j, k, n;
  Padjlist alink;
...
/**
 * function to determine if a closed link connects to junction i.
 */
static void
getclosedlink (Project * pr,
               int i,           ///< junction index.
               char *marked)    ///< marks nodes already examined.
{
...
/**
 * function to test current hydraulic solution to see if any closed links have
 * caused the network to become disconnected.
 *
 * \return number of disconnected nodes.
 */
static int
disconnected (Project * pr)
{
  Network *net = &pr->network;
  ...
  marknodes (net, hyd, mcount, nodelist, marked);
  j = 0;
  count = 0;
  for (i = 1; i <= net->Njuncs; i++)
    {
...
/**
 * function to check if output reporting criteria is met.
 * \return 1 if criteria met, 0 otherwise.
 */
static int
checklimits (Report * rpt,
             double *y,         ///< array of output results.
             int j1,            ///< index of first output variable.
             int j2)            ///< index of last output variable.
{
  int j;
  for (j = j1; j <= j2; j++)
    {
      if (y[j] > rpt->Field[j].RptLim[LOW] || y[j] < rpt->Field[j].RptLim[HI])
        return 0;
    }
  return 1;
}
...
/**
 * function to fill n bytes of s to character ch.
 * NOTE: does not check for overwriting s.
 */
static void
fillstr (char *s,               ///< string to fill.
         char ch,               ///< character to fill.
...
/**
 * function to determine type of node with index i.
 * 
 * \return junction = 0, reservoir = 1, tank = 2.
 */
static int
getnodetype (Network * net,
             int i)             ///< index.
{
  if (i <= net->Njuncs)
    return 0;
...
/**
 * function to write a line of output to report file.
 */
void
writeline (Project * pr,
           char *s)             ///< text string.
{
  Report *rpt = &pr->report;
...
/**
 * function to write column headings for output report tables.
 */
void
writeheader (Project * pr,
             int type,          ///< table type.
             int contin)        ///< table continuation flag.
{
  Report *rpt = &pr->report;
  ...
  if (rpt->Rptflag && rpt->LineNum + 11 > (long) rpt->PageSize)
    {
      while (rpt->LineNum < (long) rpt->PageSize)
        writeline (pr, " ");
    }
  writeline (pr, " ");
  ...
  if (type == NODEHDR)
    {
      if (rpt->Tstatflag == RANGE)
        ...
        sprintf (s, FMT78, clocktime (rpt->Atime, time->Htime));
      if (contin)
        strcat (s, t_CONTINUED);
      writeline (pr, s);
  ...
  if (type == LINKHDR)
    {
      if (rpt->Tstatflag == RANGE)
        ...
        sprintf (s, FMT81, clocktime (rpt->Atime, time->Htime));
      if (contin)
        strcat (s, t_CONTINUED);
      writeline (pr, s);
...
/**
 * function to write node results for current time to report file.
 */
static inline void
writenodetable (Project * pr,
                Pfloat * x)     ///< pointer to node results for current time.
{
  Network *net = &pr->network;
  Report *rpt = &pr->report;
              ...
              if (rpt->Field[j].Enabled == TRUE)
                {
                  if (fabs (y[j]) > 1.e6)
                    ...
                    sprintf (s1, "%10.*f", rpt->Field[j].Precision, y[j]);
                  strcat (s, s1);
                }
            }
...
/**
 * function to write program logo to report file.
 */
void
writelogo (Project * pr)
{
  Report *rpt = &pr->report;
...
/**
 * function to write energy usage report to report file.
 */
static void
writeenergy (Project * pr)
{
  Network *net = &pr->network;
  Hydraul *hyd = &pr->hydraul;
  Report *rpt = &pr->report;
...
/**
 * function to write link results for current time to report file.
 */
static inline void
writelinktable (Project * pr,
                Pfloat * x)     ///< pointer to link results for current time.
{
  Network *net = &pr->network;
  Report *rpt = &pr->report;
  ...
  for (i = 1; i <= net->Nlinks; i++)
    {
      ...
      if ((rpt->Linkflag == 1 || Link[i].Rpt)
          && checklimits (rpt, y, DIAM, FRICTION))
        {
          // Check if new page needed
          if (rpt->LineNum == (long) rpt->PageSize)
...
/**
 * function to write simulation results to report file.
 *
 * \return error code.
 */
static int
writeresults (Project * pr)
{
  Network *net = &pr->network;
  ...
  int j, m, n, np,              // Reporting period counter
  ...
  if (!rpt->Nodeflag && !rpt->Linkflag)
    return errcode;

  nnv = 0;
  for (j = ELEV; j <= QUALITY; j++)
    nnv += rpt->Field[j].Enabled;
  nlv = 0;
  for (j = LENGTH; j <= FRICTION; j++)
    nlv += rpt->Field[j].Enabled;
  if (nnv == 0 && nlv == 0)
    return errcode;

  // Return if no output file
  if (outFile == NULL)
    outFile = fopen (pr->outfile.OutFname, "rb");
  if (outFile == NULL)
    return 106;
...
/**
 * fucntion to write formatted output report to file.
 *
 * \return error code.
 */
int
writereport (Project * pr)
{
  Report *rpt = &pr->report;
  Parser *parser = &pr->parser;
...
/**
 * function to write reporting criteria to output report.
 */
static void
writelimits (Project * pr,
             int j1,            ///< index of first output variable.
             int j2)            ///< index of last output variable.
{
  Report *rpt = &pr->report;
      ...
      if (rpt->Field[j].RptLim[LOW] < BIG)
        {
          sprintf (pr->Msg, FMT47, rpt->Field[j].Name,
                   rpt->Field[j].RptLim[LOW], rpt->Field[j].Units);
          writeline (pr, pr->Msg);
        }
      if (rpt->Field[j].RptLim[HI] > -BIG)
        {
          sprintf (pr->Msg, FMT48, rpt->Field[j].Name,
                   rpt->Field[j].RptLim[HI], rpt->Field[j].Units);
          writeline (pr, pr->Msg);
        }
    }
}
...
/**
 * function to write summary system information to report file.
 */
void
writesummary (Project * pr)
{
  Network *net = &pr->network;
  Hydraul *hyd = &pr->hydraul;
  Quality *qual = &pr->quality;
  ...
  if (hyd->HeadErrorLimit > 0.0)
    {
      sprintf (s, FMT27d, hyd->HeadErrorLimit * pr->Ucf[HEAD],
               rpt->Field[HEAD].Units);
      writeline (pr, s);
    }
  if (hyd->FlowChangeLimit > 0.0)
    {
      sprintf (s, FMT27e, hyd->FlowChangeLimit * pr->Ucf[FLOW],
               rpt->Field[FLOW].Units);
      writeline (pr, s);
    }
  ...
  if (qual->Qualflag == NONE || time->Dur == 0.0)
    sprintf (s, FMT29);
  else if (qual->Qualflag == CHEM)
    sprintf (s, FMT30, qual->ChemName);
  else if (qual->Qualflag == TRACE)
    sprintf (s, FMT31, net->Node[qual->TraceNode].ID);
  ...
  if (qual->Qualflag != NONE && time->Dur > 0)
    {
      ...
      writeline (pr, s);
      sprintf (s, FMT34, qual->Ctol * pr->Ucf[QUALITY],
               rpt->Field[QUALITY].Units);
      writeline (pr, s);
    }
...
/**
 * function to write hydraulic status report for solution found at current time
 * period to report file.
 */
void
writehydstat (Project * pr,
              int iter,         ///< # iterations to find hydraulic solution.
              double relerr)    ///< convergence error in hydraulic solution.
{
  Network *net = &pr->network;
  Hydraul *hyd = &pr->hydraul;
  Report *rpt = &pr->report;
  ...
  for (i = 1; i <= net->Ntanks; i++)
    {
      ...
      if (ABS (NodeDemand[n]) < 0.001)
        ...
        newstat = hyd->OldStatus[net->Nlinks + i];
      if (newstat != hyd->OldStatus[net->Nlinks + i])
        {
          if (Tank[i].A > 0.0)
            {
              snprintf (s1, MAXLINE, FMT50, atime, net->Node[n].ID,
                        StatTxt[newstat],
            ...
            {
              snprintf (s1, MAXLINE, FMT51, atime, net->Node[n].ID,
                        StatTxt[newstat]);
            }
          writeline (pr, s1);
...
/**
 * function to write water quality mass balance ratio
 * (Outflow + Final Storage) / (Inflow + Initial Storage)
 * to report file.
 */
void
writemassbalance (Project * pr)
{
  Quality *qual = &pr->quality;
  ...
  if (qual->Qualflag == TRACE)
    ...
    {
      if (match (qual->ChemUnits, "mg"))
        kunits = 1;
      else if (match (qual->ChemUnits, "ug"))
        kunits = 2;
    }
...
/**
 * function to write out convergence status of hydraulic solution.
 */
void
writerelerr (Project * pr,
             int iter,          ///< current iteration of hydraulic solution.
             double relerr)     ///< current convergence error.
{
  Report *rpt = &pr->report;
  Times *time = &pr->times;
...
/**
 * function to write change in link status to output report.
 */
void
writestatchange (Project * pr,
                 int k,         ///< link index.
                 char s1,       ///< old link status.
                 char s2)       ///< new link status.
{
  Network *net = &pr->network;
  Hydraul *hyd = &pr->hydraul;
...
/**
 * function to write control action taken to status report.
 */
void
writecontrolaction (Project * pr,
                    int k,      ///< link index.
                    int i)      ///< control index.
{
  Network *net = &pr->network;
...
/**
 * function to  write rule action taken to status report.
 */
void
writeruleaction (Project * pr,
                 int k,         ///< link index.
                 char *ruleID)  ///< rule ID.
{
  Network *net = &pr->network;
...
/**
 * function to write hydraulic warning message to report file.
 *
 ...
 *   6. System unbalanced
 *
 * \return warning flag code.
 */
int
writehydwarn (Project * pr,
              int iter,         ///< # iterations to find hydraulic solution.
              double relerr)
{
  Network *net = &pr->network;
  ...
  for (i = 1; i <= net->Npumps; i++)
    {
      ...
      if (hyd->LinkStatus[j] >= OPEN)
        {
          if (hyd->LinkFlow[j] > hyd->LinkSetting[j] * pump->Qmax)
            s = XFLOW;
          if (hyd->LinkFlow[j] < 0.0)
            s = XHEAD;
        }
      if (s == XHEAD || s == XFLOW)
        {
...
/**
 * function to output status & checks connectivity when network hydraulic
 * equations cannot be solved.
 */
void
writehyderr (Project * pr,
             int errnode)
{
  Network *net = &pr->network;
  Report *rpt = &pr->report;
...
/**
 * function to write starting/ending time of a run to report file.
 */
void
writetime (Project * pr,
