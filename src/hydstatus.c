/**
 * \file hydstatus.c
 * \brief Source file to update hydraulic status of network elements.
 * \author see AUTHORS.
...
/**
 * function to update status of a check valve link.
 *
 * \return new link status.
 */
static StatusType
cvstatus (Hydraul * hyd,
          StatusType s,         ///< current link status.
          double dh,            ///< head loss across link.
          double q)             ///< link flow.
{
...
/**
 * function to update status of an open pump.
 *
 * \return new pump status.
 */
static inline StatusType
pumpstatus (Network * net,
            Hydraul * hyd,
            int k,              ///< link index.
            double dh)          ///< head gain across link.
{
...
/**
 * function to update status of a pressure reducing valve.
 *
 * \return new valve status.
 */
static inline StatusType
prvstatus (Network * net,
           Hydraul * hyd,
           int k,               ///< link index.
           StatusType s,        ///< current status.
           double hset,         ///< valve head setting.
           double h1,           ///< head at upstream node.
           double h2)           ///< head at downstream node.
{
...
/*
 * function to update status of a pressure sustaining valve.
 *
 * \return new valve status.
 */
static inline StatusType
psvstatus (Network * net,
           Hydraul * hyd,
           int k,               ///< link index.
           StatusType s,        ///< current status.
           double hset,         ///< valve head setting.
...
/**
 * function to update status of a flow control valve.
 *
 * \return new valve status.
 *
 * Valve status changes to XFCV if flow reversal. If current status is XFCV and
 * current flow is above setting, then valve becomes active. If current status
 * is XFCV, and current flow positive but still below valve setting, then status
 * remains same.
*/
static inline StatusType
fcvstatus (Hydraul * hyd,
           int k,               ///< link index.
           StatusType s,        ///< current status.
           double h1,           ///< head at upstream node.
           double h2)           ///< head at downstream node.
...
/**
 * function to close link flowing into full or out of empty tank.
 */
static inline void
tankstatus (Network * net,
            Hydraul * hyd,
            int k,              ///< link index.
            int n1,             ///< start node of link.
            int n2)             ///< end node of link.
{
  int i, n;
  double h, q;
  Stank *tank;
  ...
  if (hyd->NodeHead[n1] >= tank->Hmax - hyd->Htol && !tank->CanOverflow)
    {
      ...
      else if (cvstatus (hyd, OPEN, h, q) == CLOSED)
        {
          hyd->LinkStatus[k] = TEMPCLOSED;
        }
    }
  ...
  if (hyd->NodeHead[n1] <= tank->Hmin + hyd->Htol)
    {
      ...
      else if (cvstatus (hyd, CLOSED, h, q) == OPEN)
        {
          hyd->LinkStatus[k] = TEMPCLOSED;
        }
    }
  ...
  for (i = 1; i <= net->Nvalves; i++)
    {
      ...
      switch (link->Type)
        ...
        case PRV:
          ...
          hyd->LinkStatus[k] = prvstatus (net, hyd, k, status, hset,
                                          hyd->NodeHead[n1], hyd->NodeHead[n2]);
          break;
        case PSV:
          hset = net->Node[n1].El + hyd->LinkSetting[k];
          hyd->LinkStatus[k] = psvstatus (net, hyd, k, status, hset,
                                          hyd->NodeHead[n1], hyd->NodeHead[n2]);
          break;
        default:
          continue;
  ...
  for (k = 1; k <= net->Nlinks; k++)
    {
      ...
      if (link->Type == CVPIPE)
        {
          hyd->LinkStatus[k] = cvstatus (hyd, hyd->LinkStatus[k], dh,
                                         hyd->LinkFlow[k]);
        }
      if (link->Type == PUMP && hyd->LinkStatus[k] >= OPEN &&
          hyd->LinkSetting[k] > 0.0)
        {
          hyd->LinkStatus[k] = pumpstatus (net, hyd, k, -dh);
        }

      // Check for status changes in non-fixed FCVs
      if (link->Type == FCV && hyd->LinkSetting[k] != MISSING)
        {
          hyd->LinkStatus[k] = fcvstatus (hyd, k, status, hyd->NodeHead[n1],
                                          hyd->NodeHead[n2]);
        }
