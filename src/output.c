...
size_t
f_save (REAL4 * x, int n, FILE * file)
{
  return fwrite (x + 1, sizeof (REAL4), n, file);
}

size_t
f_read (REAL4 * x, int n, FILE * file)
{
  return fread (x + 1, sizeof (REAL4), n, file);
}

int
savenetdata (Project * pr)
/*
**---------------------------------------------------------------
**   Input:   none
...
int
savehydstep (Project * pr, long *hydstep)
/*
**--------------------------------------------------------------
  ...
  for (i = 1; i <= net->Npumps; i++)
    {
      ...
      if (parser->Unitsflag == SI)
        ...
        pump->Energy.KwHrsPerFlow *= (1.0e6 / GPMperCFS / 60.);
...
int
readhyd (Project * pr, long *hydtime)
/*
**--------------------------------------------------------------
**   Input:   none
...
*/
{
  ...
  *hydtime = t;
  ...
  if (f_read (x, net->Nnodes, HydFile) < (unsigned) net->Nnodes)
    result = 0;
  else
    for (i = 1; i <= net->Nnodes; i++)
  ...
  if (f_read (x, net->Nlinks, HydFile) < (unsigned) net->Nlinks)
    result = 0;
  else
    for (i = 1; i <= net->Nlinks; i++)
      hyd->LinkFlow[i] = x[i];
...
int
readhydstep (Project * pr, long *hydstep)
/*
**--------------------------------------------------------------
**   Input:   none
...
int
nodeoutput (Project * pr, int j, REAL4 * x, double ucf)
/*
**--------------------------------------------------------------
...
*/
{
  ...
  if (f_save (x, net->Nnodes, outFile) < (unsigned) net->Nnodes)
    return 308;
  return 0;
}

int
linkoutput (Project * pr, int j, REAL4 * x, double ucf)
/*
**----------------------------------------------------------------
...
int
savetimestat (Project * pr, REAL4 * x, HdrType objtype)
/*
**--------------------------------------------------------------
    ...
    for (j = n1; j <= n2; j++)
      {
        ...
        switch (rpt->Tstatflag)
          ...
          case AVG:
            for (i = 1; i <= n; i++)
              x[i] = stat1[i] / (float) rpt->Nperiods;
            break;
          case MIN:
            for (i = 1; i <= n; i++)
              x[i] = stat1[i];
        ...
        if (f_save (x, n, outFile) < (unsigned) n)
          errcode = 308;
        ...
        if (objtype == NODEHDR)
          switch (j)
            ...
            case QUALITY:
              for (i = 1; i <= n; i++)
                qual->NodeQual[i] = x[i] / pr->Ucf[QUALITY];
              break;
            }
        else if (j == FLOW)
          {
            for (i = 1; i <= n; i++)
              hyd->LinkFlow[i] = x[i] / pr->Ucf[FLOW];
          }
      }
...
int
savenetreacts (Project * pr, double wbulk, double wwall, double wtank,
               double wsource)
/*
**-----------------------------------------------------
