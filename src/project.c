...
int
openfiles (Project * pr, const char *f1, const char *f2, const char *f3)
/*----------------------------------------------------------------
...
*/
{
  ...
  if (strlen (f1) > 0 && (strcomp (f1, f2) || strcomp (f1, f3)))
    return 301;
  if (strlen (f3) > 0 && strcomp (f2, f3))
    return 301;

  // Attempt to open input and report files
  if (strlen (f1) > 0)
    {
      if ((pr->parser.InFile = fopen (f1, "rt")) == NULL)
        return 302;
    }
  if (strlen (f2) == 0)
  ...
  if (pr->outfile.Hydflag == USE)
    {
      ...
      if (version != ENGINE_VERSION)
        return 306;
      if (fread (nsize, sizeof (INT4), 6, pr->outfile.HydFile) < 6)
        return 306;
  ...
  if (!errcode)
    {
      if (pr->report.Tstatflag != SERIES)
        ...
        pr->outfile.TmpOutFile = pr->outfile.OutFile;
    }
  return errcode;
}

void
closeoutfile (Project * pr)
/*----------------------------------------------------------------
**  Input:   none
**  Output:  none
...
Pdemand
finddemand (Pdemand d, int index)
/*----------------------------------------------------------------
...
int
adddemand (Snode * node, double dbase, int dpat, char *dname)
/*----------------------------------------------------------------
...
*/
{
  ...
  if (dname && strlen (dname) > 0)
    xstrcpy (&demand->Name, dname, MAXID);
  demand->next = NULL;
  ...
  // Otherwise append this demand to the end of the node's demands list
    ...
    {
      lastdemand = node->D;
      while (lastdemand->next)
        lastdemand = lastdemand->next;
...
int
addlinkvertex (Slink * link, double x, double y)
/*----------------------------------------------------------------
...
*/
{
  ...
  if (vertices->X == NULL || vertices->Y == NULL)
    return 101;
  n = vertices->Npts;
  vertices->X[n] = x;
  vertices->Y[n] = y;
...
int
incontrols (Project * pr, int objType, int index)
/*----------------------------------------------------------------
...
*/
{
  ...
  for (i = 1; i <= net->Ncontrols; i++)
    {
      if (objType == NODE && net->Control[i].Node == index)
        return 1;
      if (objType == LINK && net->Control[i].Link == index)
        return 1;
    }
  ...
  for (i = 1; i <= net->Nrules; i++)
    {
      ...
      while (premise != NULL)
        {
          if (ruleObject == premise->object && premise->index == index)
            return 1;
...
int
valvecheck (Project * pr, int index, int type, int j1, int j2)
/*
**--------------------------------------------------------------
...
int
findnode (Network * network, char *id)
/*----------------------------------------------------------------
**  Input:   id = node ID
**  Output:  none
...
int
findlink (Network * network, char *id)
/*----------------------------------------------------------------
**  Input:   id = link ID
**  Output:  none
...
int
findtank (Network * network, int index)
/*----------------------------------------------------------------
...
int
findpump (Network * network, int index)
/*----------------------------------------------------------------
**  Input:   index = link ID
**  Output:  none
...
int
findvalve (Network * network, int index)
/*----------------------------------------------------------------
**  Input:   index = link ID
**  Output:  none
...
int
findpattern (Network * network, char *id)
/*----------------------------------------------------------------
...
*/
{
  ...
  for (i = 0; i <= network->Npats; i++)
    {
      if (strcmp (id, network->Pattern[i].ID) == 0)
        return i;
    }
  return -1;
}
...
int
findcurve (Network * network, char *id)
/*----------------------------------------------------------------
...
*/
{
  ...
  for (i = 1; i <= network->Ncurves; i++)
    {
      if (strcmp (id, network->Curve[i].ID) == 0)
        return i;
    }
  return 0;
}
...
void
adjustpattern (int *pat, int index)
/*----------------------------------------------------------------
...
void
adjustpatterns (Network * network, int index)
/*----------------------------------------------------------------
  ...
  for (j = 1; j <= network->Nnodes; j++)
    {
      ...
      if (source)
        adjustpattern (&source->Pat, index);
    }
...
void
adjustcurve (int *curve, int index)
/*----------------------------------------------------------------
...
void
adjustcurves (Network * network, int index)
/*----------------------------------------------------------------
...
int
adjustpumpparams (Project * pr, int curveIndex)
/*----------------------------------------------------------------
...
int
resizecurve (Scurve * curve, int size)
/*----------------------------------------------------------------
...
int
getcomment (Network * network, int object, int index, char *comment)
//----------------------------------------------------------------
...
int
setcomment (Network * network, int object, int index, const char *newcomment)
//----------------------------------------------------------------
//  Input:   object = a type of network object
//           index = index of the specified object
...
//----------------------------------------------------------------
{
  ...
  if (n < 1 || n > MAXID || strpbrk (name, " ;") || name[0] == '"')
    return FALSE;
  return TRUE;
}
...
char *
xstrcpy (char **s1, const char *s2, const size_t n)
//----------------------------------------------------------------
...
//----------------------------------------------------------------
{
  ...
  if (*s1)
    n1 = strlen (*s1);
  if (n2 > n1)
    *s1 = realloc (*s1, (n2 + 1) * sizeof (char));
...
int
strcomp (const char *s1, const char *s2)
/*---------------------------------------------------------------
...
double
interp (int n, double x[], double y[], double xx)
/*----------------------------------------------------------------
...
char *
geterrmsg (int errcode, char *msg)
/*----------------------------------------------------------------
