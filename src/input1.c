...
*/
{
  ...
  if (hyd->Viscos == MISSING)
    ...
    hyd->Viscos = hyd->Viscos / ucf;    // Actual value supplied
  if (qual->Diffus == MISSING)
    ...
    qual->Diffus = qual->Diffus / ucf;  //  Actual value supplied
  ...
  if (qual->Qualflag == NONE)
    rpt->Field[QUALITY].Enabled = FALSE;
}

int
inittanks (Project * pr)
/*
**---------------------------------------------------------------
**  Input:   none
**  Output:  returns error code
  ...
  for (j = 1; j <= net->Ntanks; j++)
    {
      ...
      if (tank->H0 > tank->Hmax ||
          tank->Hmin > tank->Hmax || tank->H0 < tank->Hmin)
        levelerr = 1;
  ...
  if (parser->Unitsflag == SI)  // SI units
    {
      ...
      if (parser->Pressflag == METERS)
        strcpy (rpt->Field[PRESSURE].Units, u_METERS);
      else
        strcpy (rpt->Field[PRESSURE].Units, u_KPA);
      ...
      dcf = 1000.0 * MperFT;
      qcf = LPSperCFS;
      if (parser->Flowflag == LPM)
        qcf = LPMperCFS;
      if (parser->Flowflag == MLD)
        qcf = MLDperCFS;
      if (parser->Flowflag == CMH)
        qcf = CMHperCFS;
      if (parser->Flowflag == CMD)
        qcf = CMDperCFS;
  ...
  else                          // US units
    {
      ...
      dcf = 12.0;
      qcf = 1.0;
      if (parser->Flowflag == GPM)
        qcf = GPMperCFS;
      if (parser->Flowflag == MGD)
        qcf = MGDperCFS;
      if (parser->Flowflag == IMGD)
        qcf = IMGDperCFS;
      if (parser->Flowflag == AFD)
        qcf = AFDperCFS;
      hcf = 1.0;
