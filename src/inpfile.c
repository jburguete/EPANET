      ...
      else
        {
          ...
          switch (sect)
            ...
            case _TAGS:
              if (*tok == ';' ||
                  (match ("NODE", tok)
                   && findnode (&pr->network, strtok (NULL, SEPSTR)))
                  || (match ("LINK", tok)
                      && findlink (&pr->network, strtok (NULL, SEPSTR))))
                write = TRUE;
              break;
            case _LABELS:
            case _BACKDROP:
...
int
saveinpfile (Project * pr, const char *fname)
/*
-------------------------------------------------
...
*/
{
      ...
      // Old format used for pump curve
      else
        {
          fprintf (f, "\n%s %12.4f %12.4f %12.4f          0.0 %12.4f", s,
                   -pump->H0 * pr->Ucf[HEAD],
                   (-pump->H0 -
                    pump->R * pow (pump->Q0, pump->N)) * pr->Ucf[HEAD],
  ...
  for (i = 1; i <= net->Njuncs; i++)
    {
      ...
      if (node->Ke == 0.0)
        continue;
      ke =
        pr->Ucf[FLOW] / pow (pr->Ucf[PRESSURE] * node->Ke, (1.0 / hyd->Qexp));
  ...
  fprintf (f, "\n REPORT START        %s",
           clocktime (rpt->Atime, time->Rstart));
  fprintf (f, "\n PATTERN TIMESTEP    %s", clocktime (rpt->Atime, time->Pstep));
  fprintf (f, "\n PATTERN START       %s",
           clocktime (rpt->Atime, time->Pstart));
  fprintf (f, "\n RULE TIMESTEP       %s",
           clocktime (rpt->Atime, time->Rulestep));
  fprintf (f, "\n START CLOCKTIME     %s",
           clocktime (rpt->Atime, time->Tstart));
