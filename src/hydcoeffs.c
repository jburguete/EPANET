  ...
  for (k = 1; k <= net->Nlinks; k++)
    {
      ...
      // ... node n1 is a tank/reservoir
        ...
        sm->F[sm->Row[n2]] += (hyd->P[k] * hyd->NodeHead[n1]);
      ...
      // ... node n2 is a tank/reservoir
        ...
        sm->F[sm->Row[n1]] += (hyd->P[k] * hyd->NodeHead[n2]);
    }
}


void
nodecoeffs (Project * pr)
/*
**----------------------------------------------------------------
**  Input:   none
**  Output:  none
...
void
emitterheadloss (Project * pr, int i, double *hloss, double *hgrad)
/*
**-------------------------------------------------------------
**   Input:   i = node index
...
void
demandheadloss (Project * pr, int i, double dp, double n,
...
double
frictionFactor (double q, double e, double s, double *dfdq)
/*
**--------------------------------------------------------------
**   Input:   q = |pipe flow|
...
void
curvecoeff (Project * pr, int i, double q, double *h0, double *r)
/*
**-------------------------------------------------------------------
**   Input:   i   = curve index
**            q   = flow rate
...
void
prvcoeff (Project * pr, int k, int n1, int n2)
/*
**--------------------------------------------------------------
**   Input:   k    = link index
...
*/
{
  ...
  j = sm->Row[n2];
  hset = pr->network.Node[n2].El + hyd->LinkSetting[k]; // Valve setting
...
void
psvcoeff (Project * pr, int k, int n1, int n2)
/*
**--------------------------------------------------------------
**   Input:   k    = link index
**            n1   = upstream node of valve
...
*/
{
  ...
  i = sm->Row[n1];              // Matrix rows of nodes
  j = sm->Row[n2];
  hset = pr->network.Node[n1].El + hyd->LinkSetting[k]; // Valve setting
...
void
fcvcoeff (Project * pr, int k, int n1, int n2)
/*
**--------------------------------------------------------------
**   Input:   k    = link index
**            n1   = upstream node of valve
