...
*/
{
  ...
  if (!errcode)
    {
      if (parser->MaxJuncs < 1)
        errcode = 223;          // Not enough nodes
      else if (parser->MaxTanks == 0)
  ...
  while (fgets (line, MAXLINE, parser->InFile) != NULL)
    {
      ...
      strcpy (wline, line);
      parser->Ntokens =
        gettokens (wline, parser->Tok, MAXTOKS, parser->Comment);
...
int
newline (Project * pr, int sect, char *line)
/*
**--------------------------------------------------------------
...
int
updatepumpparams (Project * pr, int pumpindex)
/*
**-------------------------------------------------------------
...
int
addnodeID (Network * net, int n, char *id)
/*
**-------------------------------------------------------------
**  Input:   n = node index
**           id = ID label
...
*/
{
  ...
  g_hash_table_insert (net->NodeHashTable, net->Node[n].ID,
                       GINT_TO_POINTER (n));
  return 0;
}

int
addlinkID (Network * net, int n, char *id)
/*
**-------------------------------------------------------------
**  Input:   n = link index
**           id = ID label
...
*/
{
  ...
  g_hash_table_insert (net->LinkHashTable, net->Link[n].ID,
                       GINT_TO_POINTER (n));
  return 0;
}

int
addpattern (Network * network, char *id)
/*
**-------------------------------------------------------------
...
*/
{
  ...
  n = n + 2;
  network->Pattern =
    (Spattern *) realloc (network->Pattern, n * sizeof (Spattern));
...
int
addcurve (Network * network, char *id)
/*
**-------------------------------------------------------------
  ...
  for (i = 1; i <= net->Njuncs; i++)
    {
      ...
      if (marked[i] == 0)
        {
          err++;
          sprintf (pr->Msg, "Error 233: %s %s", geterrmsg (233, pr->Msg),
                   net->Node[i].ID);
          writeline (pr, pr->Msg);
        }
      if (err >= MAXERRS)
...
int
match (const char *str, const char *substr)
/*
**--------------------------------------------------------------
...
int
gettokens (char *s, char **Tok, int maxToks, char *comment)
/*
 **--------------------------------------------------------------
...
void
inperrmsg (Project * pr, int err, int sect, char *line)
/*
**-------------------------------------------------------------
