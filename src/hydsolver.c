...
int
hydsolve (Project * pr, int *iter, double *relerr)
/*
**-------------------------------------------------------------------
**  Input:   none
  ...
  while (*iter <= maxtrials)
    {
      ...
      if (errcode > 0)
        {
          if (badvalve (pr, sm->Order[errcode]))
            continue;
          else
            break;
        }
      ...
      if (reset == 1)
        {
          ...
          if (link->Type == PIPE)
            {
              if (s != (char) net->Control[i].Status)
                change = 1;
            }
          if (link->Type == PUMP)
            {
...
double
newflows (Project * pr, Hydbalance * hbal)
/*
**----------------------------------------------------------------
...
void
newlinkflows (Project * pr, Hydbalance * hbal, double *qsum, double *dqsum)
/*
**----------------------------------------------------------------
**  Input:   hbal = ptr. to hydraulic balance information
  ...
  for (k = 1; k <= net->Nlinks; k++)
    {
      ...
      *dqsum += ABS (dq);
      ...
      if (hyd->LinkStatus[k] > CLOSED)
        {
          if (n1 > net->Njuncs)
            hyd->NodeDemand[n1] -= hyd->LinkFlow[k];
          if (n2 > net->Njuncs)
            hyd->NodeDemand[n2] += hyd->LinkFlow[k];
        }
    }
}
...
void
newemitterflows (Project * pr, Hydbalance * hbal, double *qsum, double *dqsum)
/*
**----------------------------------------------------------------
**  Input:   hbal = ptr. to hydraulic balance information
**           qsum = sum of current system flows
...
void
newdemandflows (Project * pr, Hydbalance * hbal, double *qsum, double *dqsum)
/*
**----------------------------------------------------------------
**  Input:   hbal = ptr. to hydraulic balance information
**           qsum = sum of current system flows
...
void
checkhydbalance (Project * pr, Hydbalance * hbal)
/*
**--------------------------------------------------------------
...
int
hasconverged (Project * pr, double *relerr, Hydbalance * hbal)
/*
**--------------------------------------------------------------
...
*/
{
  ...
  if (hyd->HeadErrorLimit > 0.0 && hbal->maxheaderror > hyd->HeadErrorLimit)
    return 0;
  if (hyd->FlowChangeLimit > 0.0 && hbal->maxflowchange > hyd->FlowChangeLimit)
    return 0;

  // Check for pressure driven analysis convergence
  if (hyd->DemandModel == PDA)
    return pdaconverged (pr);
  return 1;
}

int
pdaconverged (Project * pr)
/*
**--------------------------------------------------------------
**   Input:   none
...
void
reporthydbal (Project * pr, Hydbalance * hbal)
/*
**--------------------------------------------------------------
