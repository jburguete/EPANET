...
*/
{
  ...
  if (pr->quality.Qualflag == TRACE)
    return 0;
  else if (pr->quality.Qualflag == AGE)
    return 1;
  else
    {
  ...
  for (k = 1; k <= net->Nlinks; k++)
    {
      ...
      if (vsum > 0.0)
        qual->PipeRateCoeff[k] = rsum / vsum / dt * SECperDAY;
      else
        qual->PipeRateCoeff[k] = 0.0;
    }
}
...
double
pipereact (Project * pr, int k, double c, double v, long dt)
/*
**------------------------------------------------------------
**   Input:   k = link index
...
double
tankreact (Project * pr, double c, double v, double kb, long dt)
/*
**-------------------------------------------------------
...
double
bulkrate (Project * pr, double c, double kb, double order)
/*
**-----------------------------------------------------------
  ...
  // N-th order kinetics:
  else
    {
      ...
      if (qual->Climit == 0.0)
        ...
        c1 = MAX (0.0, SGN (kb) * (qual->Climit - c));
...
double
wallrate (Project * pr, double c, double d, double kw, double kf)
/*
**------------------------------------------------------------
**   Input:   c = current WQ concentration
**            d = pipe diameter
  ...
  if (qual->WallOrder == 0.0)   // 0-order reaction */
    {
      ...
      if (fabs (kf) < fabs (kw))
        kw = kf;                // Reaction mass transfer limited
...
double
mixtank (Project * pr, int n, double volin, double massin, double volout)
/*
**------------------------------------------------------------
**   Input:   n      = node index
...
*/
{
  ...
  switch (net->Tank[i].MixModel)
    {
    case MIX1:
      tankmix1 (pr, i, volin, massin, vnet);
      break;
    case MIX2:
      tankmix2 (pr, i, volin, massin, vnet);
      break;
    case FIFO:
      tankmix3 (pr, i, volin, massin, vnet);
      break;
    case LIFO:
      tankmix4 (pr, i, volin, massin, vnet);
      break;
    }
  return net->Tank[i].C;
}
...
void
tankmix1 (Project * pr, int i, double vin, double win, double vnet)
/*
**---------------------------------------------
**   Input:   i = tank index
**            vin = inflow volume
...
void
tankmix2 (Project * pr, int i, double vin, double win, double vnet)
/*
**------------------------------------------------
**   Input:   i = tank index
**            vin = inflow volume
...
void
tankmix3 (Project * pr, int i, double vin, double win, double vnet)
/*
**----------------------------------------------------------
**   Input:   i = tank index
**            vin = inflow volume
...
*/
{
  ...
  k = net->Nlinks + i;
  if (qual->LastSeg[k] == NULL || qual->FirstSeg[k] == NULL)
    return;
  ...
  while (vout > 0.0)
    {
      ...
      if (vout >= 0.0 && vseg >= seg->v)        // Seg used up
        ...
        seg->v -= vseg;         // Remaining volume in segment
    }
...
void
tankmix4 (Project * pr, int i, double vin, double win, double vnet)
/*
**----------------------------------------------------------
**   Input:   i = tank index
**            vin = inflow volume
