...
typedef struct
{
  ...
  long Tstart,                  // Starting time of day
...
typedef struct
{
  ...
  char Atime[13],               // Clock time (hrs:min:sec)
...
typedef struct
{
  ...
  double *NodeHead,             // Node hydraulic heads
  ...
  int Epat,                     // Energy cost time pattern
    ...
    StatusType * LinkStatus,    // Link status
...
typedef struct
{
  ...
  int Qualflag,                 // Water quality analysis flag
   ...
   *LastSeg;                    // Last (upstream) segment in each pipe
    ...
    SmassBalance MassBalance;   // Mass balance components
...
typedef struct
{
  ...
  Srule *Rule;                  // Rule-based controls array
    GHashTable * NodeHashTable, // Hash table for Node ID names
...
typedef struct Project
{
  ...
  int Openflag,                 // Project open flag
    Warnflag;                   // Warning flag

  char Msg[MAXMSG + 1],         // General-purpose string: errors, messages
