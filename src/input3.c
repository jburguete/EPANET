...
int
setError (Parser * parser, int tokindex, int errcode)
/*
**--------------------------------------------------------------
...
*/
{
  ...
  if (err)
    return setError (parser, 0, err);

  // Check for valid data
  if (n < 2)
    return 201;
  if (!getfloat (parser->Tok[1], &el))
    return setError (parser, 1, 202);
  if (n >= 3 && !getfloat (parser->Tok[2], &y))
    return setError (parser, 2, 202);
  if (n >= 4)
    {
      p = findpattern (net, parser->Tok[3]);
      if (p < 0)
        return setError (parser, 3, 205);
    }
...
*/
{
  ...
  n = parser->Ntokens;
  if (net->Ntanks == parser->MaxTanks || net->Nnodes == parser->MaxNodes)
    return 200;
  net->Ntanks++;
  net->Nnodes++;
  ...
  // Tank is a storage tank
    ...
    {
      if (!getfloat (parser->Tok[2], &initlevel))
        return setError (parser, 2, 202);
      if (!getfloat (parser->Tok[3], &minlevel))
        return setError (parser, 3, 202);
      if (!getfloat (parser->Tok[4], &maxlevel))
        return setError (parser, 4, 202);
      if (!getfloat (parser->Tok[5], &diam))
        return setError (parser, 5, 202);
      if (n >= 7 && !getfloat (parser->Tok[6], &minvol))
      ...
      if (n >= 9)
        {
          if (match (parser->Tok[8], w_YES))
            overflow = TRUE;
          else if (match (parser->Tok[8], w_NO))
            overflow = FALSE;
          else
            return setError (parser, 8, 213);
        }

      if (initlevel < 0.0)
        return setError (parser, 2, 209);
      if (minlevel < 0.0)
        return setError (parser, 3, 209);
      if (maxlevel < 0.0)
        return setError (parser, 4, 209);
      if (diam < 0.0)
        return setError (parser, 5, 209);
      if (minvol < 0.0)
        return setError (parser, 6, 209);
    }
  node = &net->Node[i];
...
*/
{
  ...
  if (!getfloat (parser->Tok[3], &length))
    return setError (parser, 3, 202);
  if (length <= 0.0)
    return setError (parser, 3, 211);
  if (!getfloat (parser->Tok[4], &diam))
    return setError (parser, 4, 202);
  if (diam <= 0.0)
    return setError (parser, 4, 211);
  if (!getfloat (parser->Tok[5], &rcoeff))
    return setError (parser, 5, 202);
  if (rcoeff <= 0.0)
    setError (parser, 5, 211);

  // Either a loss coeff. or a status is supplied
  if (n == 7)
    {
      if (match (parser->Tok[6], w_CV))
        type = CVPIPE;
      else if (match (parser->Tok[6], w_CLOSED))
        status = CLOSED;
      else if (match (parser->Tok[6], w_OPEN))
        status = OPEN;
      else if (!getfloat (parser->Tok[6], &lcoeff))
        return setError (parser, 6, 202);
    }
  ...
  if (n == 8)
    {
      if (!getfloat (parser->Tok[6], &lcoeff))
        return setError (parser, 6, 202);
      if (match (parser->Tok[7], w_CV))
        type = CVPIPE;
      else if (match (parser->Tok[7], w_CLOSED))
        status = CLOSED;
      else if (match (parser->Tok[7], w_OPEN))
        status = OPEN;
      else
        return setError (parser, 7, 213);
    }
  if (lcoeff < 0.0)
    return setError (parser, 6, 211);
...
*/
{
  ...
  n = parser->Ntokens;
  if (net->Nlinks == parser->MaxLinks || net->Npumps == parser->MaxPumps)
    return 200;
  net->Nlinks++;
  net->Npumps++;
  err = addlinkID (net, net->Nlinks, parser->Tok[0]);
  ...
  if (getfloat (parser->Tok[3], &parser->X[0]))
    {
      ...
      for (j = 4; j < n; j++)
        {
          if (!getfloat (parser->Tok[j], &parser->X[m]))
            return setError (parser, j, 202);
          m++;
        }
...
*/
{
  ...
  n = parser->Ntokens;
  if (net->Nlinks == parser->MaxLinks || net->Nvalves == parser->MaxValves)
    return 200;
  net->Nvalves++;
  net->Nlinks++;
  err = addlinkID (net, net->Nlinks, parser->Tok[0]);
  ...
  if (type == GPV)
    {
      ...
      if (c == 0)
        return setError (parser, 5, 206);
      setting = c;
      net->Curve[c].Type = HLOSS_CURVE;
      status = OPEN;
    }
  else if (!getfloat (parser->Tok[5], &setting))
    return setError (parser, 5, 202);
  if (n >= 7 && !getfloat (parser->Tok[6], &lcoeff))
    return setError (parser, 6, 202);
  ...
  if (valvecheck (pr, net->Nlinks, type, j1, j2))
    {
      if (j1 > net->Njuncs)
        return setError (parser, 1, 219);
      else if (j2 > net->Njuncs)
        return setError (parser, 2, 219);
      else
        return setError (parser, -1, 220);
    }
...
*/
{
  ...
  // Otherwise retrieve pattern from the network's Pattern array
  else
    {
      ...
      if (i <= 0)
        return setError (parser, 0, 205);
      pattern = &(net->Pattern[i]);
      if (pattern->Comment == NULL && parser->Comment[0])
        {
          pattern->Comment =
            xstrcpy (&pattern->Comment, parser->Comment, MAXMSG);
        }
    }
 ...
 */
{
  ...
  if (parser->Ntokens < 3)
    return 201;
  if ((j = findnode (net, parser->Tok[0])) == 0)
    return setError (parser, 0, 203);
...
*/
{
  ...
  else if (!adddemand (&net->Node[j], y, p, parser->Comment))
    return 101;
  return 0;
}

int
controldata (Project * pr)
/*
**--------------------------------------------------------------
**  Input:   none
**  Output:  returns error code
...
*/
{
  ...
  if (k == 0)
    return setError (parser, 1, 204);

  // Cannot control a check valve
  linktype = net->Link[k].Type;
  if (linktype == CVPIPE)
    return setError (parser, 1, 207);
  ...
  else if (linktype == GPV)
    return setError (parser, 1, 207);
  else if (!getfloat (parser->Tok[2], &setting))
    return setError (parser, 2, 202);
  ...
  if (match (parser->Tok[4], w_TIME))
    ctltype = TIMER;
  else if (match (parser->Tok[4], w_CLOCKTIME))
    ctltype = TIMEOFDAY;
  else
    {
      if (n < 8)
        return 201;
      if ((i = findnode (net, parser->Tok[5])) == 0)
        return setError (parser, 5, 203);
      if (match (parser->Tok[6], w_BELOW))
        ctltype = LOWLEVEL;
      else if (match (parser->Tok[6], w_ABOVE))
        ctltype = HILEVEL;
      else
        return setError (parser, 6, 213);
    }
  ...
  switch (ctltype)
    ...
    case TIMEOFDAY:
      if (n == 6)
        time = hour (parser->Tok[5], "");
      if (n == 7)
        time = hour (parser->Tok[5], parser->Tok[6]);
      if (time < 0.0)
        return setError (parser, 5, 213);
      break;
    case LOWLEVEL:
    case HILEVEL:
      if (!getfloat (parser->Tok[7], &level))
  ...
  net->Ncontrols++;
  if (net->Ncontrols > parser->MaxControls)
    return 200;
...
*/
{
  ...
  i = 2;
  if (match (parser->Tok[1], w_CONCEN))
    type = CONCEN;
  else if (match (parser->Tok[1], w_MASS))
    type = MASS;
  else if (match (parser->Tok[1], w_SETPOINT))
    type = SETPOINT;
  else if (match (parser->Tok[1], w_FLOWPACED))
    type = FLOWPACED;
  else
    i = 1;
  ...
  if (!getfloat (parser->Tok[i], &c0))
    {
      if (i == 1)
        return setError (parser, i, 213);
      else
        return setError (parser, i, 202);
    }
  ...
  if (n > i + 1 && strlen (parser->Tok[i + 1]) > 0 &&
      strcmp (parser->Tok[i + 1], "*") != 0)
    {
      ...
      if (p < 0)
        return setError (parser, i + 1, 205);
    }

  // Destroy any existing source assigned to node
  if (net->Node[j].S != NULL)
    free (net->Node[j].S);
...
*/
{
  ...
  if (!getfloat (parser->Tok[1], &k))
    return setError (parser, 1, 202);
  if (k < 0.0)
    return setError (parser, 1, 209);
  net->Node[j].Ke = k;
  return 0;
}
...
*/
{
  ...
  if (net->Nnodes == 0)
    return setError (parser, 0, 203);   // No nodes defined yet
  n = parser->Ntokens;
  if (n < 2)
    return 0;
  ...
  // Range of node names supplied
  else
    {
      ...
      if ((i1 = atol (parser->Tok[0])) > 0 && (i2 = atol (parser->Tok[1])) > 0)
        {
...
*/
{
  ...
  if (match (parser->Tok[0], w_ORDER))
    {
      if (!getfloat (parser->Tok[n - 1], &y))
        return setError (parser, n - 1, 202);
      if (match (parser->Tok[1], w_BULK))
        qual->BulkOrder = y;
      else if (match (parser->Tok[1], w_TANK))
        qual->TankOrder = y;
  ...
  if (match (parser->Tok[0], w_BULK))
    item = 1;
  else if (match (parser->Tok[0], w_WALL))
    item = 2;
  else if (match (parser->Tok[0], w_TANK))
    item = 3;
  else
    return setError (parser, 0, 213);
  ...
  if (item == 3)
    {
      ...
      if (n == 3)
        {
          if ((j = findnode (net, parser->Tok[1])) <= net->Njuncs)
            return 0;
  ...
  // Case where pipe rate coeffs. are being set
  else
    {
      ...
      if (n == 3)
        {
          if ((j = findlink (net, parser->Tok[1])) == 0)
            return 0;
          if (item == 1)
            net->Link[j].Kb = y;
          else
...
*/
{
  ...
  if (j <= net->Njuncs)
    return 0;
  if ((m = findmatch (parser->Tok[1], MixTxt)) < 0)
    return setError (parser, 1, 213);

  // Find mixing zone volume fraction (which can't be 0)
  v = 1.0;
  if ((m == MIX2) && (n == 3) && (!getfloat (parser->Tok[2], &v)))
    return setError (parser, 2, 202);
  if (v == 0.0)
    v = 1.0;
...
*/
{
  ...
  if (match (parser->Tok[n], w_OPEN))
    status = OPEN;
  else if (match (parser->Tok[n], w_CLOSED))
    status = CLOSED;
  else
    {
  ...
  else if ((i1 = atol (parser->Tok[0])) > 0 && (i2 = atol (parser->Tok[1])) > 0)
    {
      for (j = 1; j <= net->Nlinks; j++)
        {
          i = atol (net->Link[j].ID);
...
*/
{
  ...
  if (match (parser->Tok[n - 2], w_PRICE))
    {
      if (!getfloat (parser->Tok[n - 1], &y))
        return setError (parser, n - 1, 202);
      if (y < 0.0)
        return setError (parser, n - 1, 217);
      if (j == 0)
        hyd->Ecost = y;
      else
        Pump[j].Ecost = y;
  ...
  // Price PATTERN being set
  else if (match (parser->Tok[n - 2], w_PATTERN))
    {
      ...
      if (p < 0)
        return setError (parser, n - 1, 205);
      if (j == 0)
        hyd->Epat = p;
      else
        Pump[j].Epat = p;
  ...
  else if (match (parser->Tok[n - 2], w_EFFIC))
    {
      if (j == 0)
      ...
      else
        {
          ...
          if (c == 0)
            return setError (parser, n - 1, 206);
          Pump[j].Ecurve = c;
...
*/
{
  ...
  if (match (parser->Tok[0], w_STATUS))
    {
      if (match (parser->Tok[n], w_NO))
        rpt->Statflag = FALSE;
      if (match (parser->Tok[n], w_YES))
        rpt->Statflag = TRUE;
      if (match (parser->Tok[n], w_FULL))
        rpt->Statflag = FULL;
      return 0;
    }
  ...
  if (match (parser->Tok[0], w_SUMMARY))
    {
      if (match (parser->Tok[n], w_NO))
        rpt->Summaryflag = FALSE;
      if (match (parser->Tok[n], w_YES))
        rpt->Summaryflag = TRUE;
      return 0;
    }
  ...
  if (match (parser->Tok[0], w_MESSAGES))
    {
      if (match (parser->Tok[n], w_NO))
        rpt->Messageflag = FALSE;
      if (match (parser->Tok[n], w_YES))
        rpt->Messageflag = TRUE;
      return 0;
    }
  ...
  if (match (parser->Tok[0], w_NODE))
    {
      if (match (parser->Tok[n], w_NONE))
        rpt->Nodeflag = 0;      // No nodes
      else if (match (parser->Tok[n], w_ALL))
        rpt->Nodeflag = 1;      // All nodes
      else
        {
          if (net->Nnodes == 0)
            return setError (parser, 1, 203);
          for (i = 1; i <= n; i++)
            {
              if ((j = findnode (net, parser->Tok[i])) == 0)
                return setError (parser, i, 203);
              net->Node[j].Rpt = 1;
            }
          rpt->Nodeflag = 2;
        }
  ...
  if (match (parser->Tok[0], w_LINK))
    {
      if (match (parser->Tok[n], w_NONE))
      ...
      else
        {
          ...
          for (i = 1; i <= n; i++)
            {
              if ((j = findlink (net, parser->Tok[i])) == 0)
                return setError (parser, i, 204);
              net->Link[j].Rpt = 1;
            }
          rpt->Linkflag = 2;
        }
  ...
  if (strcomp (parser->Tok[0], t_HEADLOSS))
    i = HEADLOSS;
  else
    i = findmatch (parser->Tok[0], Fldname);
  if (i >= 0)
    {
      if (i > FRICTION)
        return setError (parser, 0, 213);
      ...
      if (parser->Ntokens < 3)
        return 201;
      if (match (parser->Tok[1], w_BELOW))
        j = LOW;
      else if (match (parser->Tok[1], w_ABOVE))
        j = HI;
      else if (match (parser->Tok[1], w_PRECISION))
        j = PREC;
      else
        return setError (parser, 1, 213);
...
*/
{
  ...
  if (match (parser->Tok[0], w_STATISTIC))
    {
      if (match (parser->Tok[n], w_NONE))
        rpt->Tstatflag = SERIES;
      else if (match (parser->Tok[n], w_NO))
        rpt->Tstatflag = SERIES;
      else if (match (parser->Tok[n], w_AVG))
        rpt->Tstatflag = AVG;
      else if (match (parser->Tok[n], w_MIN))
        rpt->Tstatflag = MIN;
      else if (match (parser->Tok[n], w_MAX))
        rpt->Tstatflag = MAX;
      else if (match (parser->Tok[n], w_RANGE))
        rpt->Tstatflag = RANGE;
      else
        return setError (parser, n, 213);
      return 0;
  ...
  if (match (parser->Tok[0], w_DURATION))
    time->Dur = t;
  else if (match (parser->Tok[0], w_HYDRAULIC))
    time->Hstep = t;
  else if (match (parser->Tok[0], w_QUALITY))
    time->Qstep = t;
  else if (match (parser->Tok[0], w_RULE))
    time->Rulestep = t;
  else if (match (parser->Tok[0], w_MINIMUM))
    return 0;                   // Not used anymore
  else if (match (parser->Tok[0], w_PATTERN))
    {
      if (match (parser->Tok[1], w_TIME))
        time->Pstep = t;
      else if (match (parser->Tok[1], w_START))
        time->Pstart = t;
      else
        return setError (parser, 1, 213);
    }
...
*/
{
  ...
  if (match (parser->Tok[0], w_UNITS))
    {
      if (n < 1)
        return 0;
      else if (match (parser->Tok[1], w_CFS))
        parser->Flowflag = CFS;
      else if (match (parser->Tok[1], w_GPM))
        parser->Flowflag = GPM;
      else if (match (parser->Tok[1], w_AFD))
        parser->Flowflag = AFD;
      else if (match (parser->Tok[1], w_MGD))
        parser->Flowflag = MGD;
      else if (match (parser->Tok[1], w_IMGD))
        parser->Flowflag = IMGD;
      else if (match (parser->Tok[1], w_LPS))
        parser->Flowflag = LPS;
      else if (match (parser->Tok[1], w_LPM))
        parser->Flowflag = LPM;
      else if (match (parser->Tok[1], w_CMH))
        parser->Flowflag = CMH;
      else if (match (parser->Tok[1], w_CMD))
        parser->Flowflag = CMD;
      else if (match (parser->Tok[1], w_MLD))
        parser->Flowflag = MLD;
      else if (match (parser->Tok[1], w_SI))
        parser->Flowflag = LPS;
      else
        return setError (parser, 1, 213);
    }
  ...
  // PRESSURE units
  else if (match (parser->Tok[0], w_PRESSURE))
    {
      if (n < 1)
        return 0;
      else if (match (parser->Tok[1], w_EXPONENT))
        return -1;
      else if (match (parser->Tok[1], w_PSI))
        parser->Pressflag = PSI;
      else if (match (parser->Tok[1], w_KPA))
        parser->Pressflag = KPA;
      else if (match (parser->Tok[1], w_METERS))
        parser->Pressflag = METERS;
      else
        return setError (parser, 1, 213);
    }
  ...
  // HEADLOSS formula
  else if (match (parser->Tok[0], w_HEADLOSS))
    {
      if (n < 1)
        return 0;
      else if (match (parser->Tok[1], w_HW))
        hyd->Formflag = HW;
      else if (match (parser->Tok[1], w_DW))
        hyd->Formflag = DW;
      else if (match (parser->Tok[1], w_CM))
        hyd->Formflag = CM;
      else
        return setError (parser, 1, 213);
    }
  ...
  // HYDRUALICS USE/SAVE file option
  else if (match (parser->Tok[0], w_HYDRAULIC))
    {
      if (n < 2)
        return 0;
      else if (match (parser->Tok[1], w_USE))
        out->Hydflag = USE;
      else if (match (parser->Tok[1], w_SAVE))
        out->Hydflag = SAVE;
      else
        return setError (parser, 1, 213);
  ...
  // Water QUALITY option
  else if (match (parser->Tok[0], w_QUALITY))
    {
      if (n < 1)
        return 0;
      else if (match (parser->Tok[1], w_NONE))
        qual->Qualflag = NONE;
      else if (match (parser->Tok[1], w_CHEM))
        qual->Qualflag = CHEM;
      else if (match (parser->Tok[1], w_AGE))
        qual->Qualflag = AGE;
      else if (match (parser->Tok[1], w_TRACE))
        qual->Qualflag = TRACE;
      else
        {
          qual->Qualflag = CHEM;
  ...
  // Hydraulics UNBALANCED option
  else if (match (parser->Tok[0], w_UNBALANCED))
    {
      if (n < 1)
        return 0;
      if (match (parser->Tok[1], w_STOP))
        hyd->ExtraIter = -1;
      else if (match (parser->Tok[1], w_CONTINUE))
        {
          if (n >= 2)
            hyd->ExtraIter = atoi (parser->Tok[2]);
          else
            hyd->ExtraIter = 0;
        }
      else
...
*/
{
  ...
  if (!getfloat (parser->Tok[nvalue], &y))
    return setError (parser, nvalue, 202);
...
int
powercurve (double h0, double h1, double h2, double q1, double q2,
