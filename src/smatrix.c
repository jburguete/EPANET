 ...
 */
static inline int
allocsmatrix (Smatrix * sm,     ///< sparse matrix struct.
 ...
 */
static inline int
alloclinsolve (Smatrix * sm,    ///< sparse matrix struct.
               int n)
{
  int errcode = 0;
 ...
 */
static inline void
xparalinks (Padjlist * Adjlist, int Nnodes)
{
  int i;
 ...
 */
static inline int
paralink (Padjlist * Adjlist, Smatrix * sm,     ///< sparse matrix struct.
 ...
 */
static inline int
localadjlists (Network * net, Smatrix * sm)     ///< sparse matrix struct.
{
  int i, j, k;
 ...
 */
static inline int
reordernodes (Network * net, Smatrix * sm)      ///< sparse matrix struct.
{
  ...
  if (adjncy && xadj && dhead && qsize && llist && marker)
    ...
    errcode = 101;              //insufficient memory
...
*/
static inline int
linked (Padjlist * Adjlist, int i,      ///< node index
        int j)                  ///< node index
{
  Padjlist alink;
  for (alink = Adjlist[i]; alink != NULL; alink = alink->next)
...
*/
static int
addlink (Padjlist * Adjlist, int i,     ///< node index.
         int j,                 ///< node index.
         int n)                 ///< link index.
{
  Padjlist alink;
 ...
 */
static inline int
newlink (Padjlist * Adjlist, Smatrix * sm,      ///< sparse matrix struct.
         Padjlist alink)
          ...
          if (!linked (Adjlist, inode, jnode))  // inode not linked to jnode
            {
              ...
              if (!addlink (Adjlist, inode, jnode, sm->Ncoeffs))
                return 0;
              if (!addlink (Adjlist, jnode, inode, sm->Ncoeffs))
                return 0;
              sm->Degree[inode]++;
              sm->Degree[jnode]++;
            }
 ...
 */
static inline int
growlist (Network * net, Smatrix * sm,  ///< sparse matrix struct.
...
*/
static inline int
factorize (Network * net, Smatrix * sm) ///< sparse matrix struct.
{
  int k, knode;
  int errcode = 0;
  Padjlist alink;
...
*/
static inline int
storesparse (Network * net, Smatrix * sm,       ///< sparse matrix struct.
...
*/
static void
transpose (int n,               ///< matrix order.
           int *il, int *jl, int *xl,   ///< sparse storage scheme for original matrix.
           int *ilt, int *jlt, int *xlt,
 ...
 */
static inline int
sortsparse (Smatrix * sm,       ///< sparse matrix struct.
            int n)              ///< number of rows in solution matrix.
{
  int i, k;
 ...
 */
int
createsparse (Network * net, Smatrix * sm)      ///< sparse matrix struct.
{
  int errcode = 0;
